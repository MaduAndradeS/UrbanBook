...
export default function PerfilEmpresa() {
  ...
  async function carregarTudo() {
    try {
      ...
      console.log("Erro de carregamento:", e);
    } finally {
  ...
  if (loadingInicial) {
    return <View style={{flex:1, justifyContent:'center'}}><ActivityIndicator size="large" color="#67C5C0" /></View>;
  }
      ...
      <ScrollView style={styles.container}>
        ...
        <View style={styles.content}>
            ...
            {agenda.map((item, i) => (
              <View key={i} style={styles.agendaCard}>
                <View style={styles.agendaInfo}>
                  <View style={styles.diaBadge}><Text style={styles.diaBadgeText}>{item.DIAS_ATIVOS}</Text></View>
          ...
          <View style={styles.servicosContainer}>
            {servicos.map((s, i) => (
              <View key={i} style={styles.servicoTag}><Text style={styles.servicoTexto}>{s.NOME}</Text></View>
            ))}
          </View>
          ...
          <View style={styles.portfolioContainer}>
            {fotosTrabalho.map((f, i) => <Image key={i} source={{ uri: f.URL }} style={styles.portfolioImg} />)}
          </View>
        </View>
      </ScrollView>
    </View>
